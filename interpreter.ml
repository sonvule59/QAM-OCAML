...
let check_equivalence (input: string) =
  ...
  if List.length failed_results = 0 then
    print_endline "Input is equivalent according to all rules."
  else
    List.iter (fun r -> match r with | Equivalent -> () | NotEquivalent msg -> print_endline msg) failed_results
