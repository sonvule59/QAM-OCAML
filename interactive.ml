    ...
    let rec find_action s patterns =
      ...
      | p::ps -> 
        if contains_substring s p then 
          ...
          let remaining_str = String.sub s (idx + String.length p) (String.length s - idx - String.length p) in
...
let rec interactive_prompt (membrane : membrane) =
  ...
  print_endline "2. Apply encoding rules";
  print_endline "3. Apply Cohere rule";
  print_endline "4. View current membrane state";
  print_endline "5. Execute membrane";
  ...
  | "2" ->
    ...
    print_endline "Updated membrane after applying encoding rules:";
    print_membrane_state updated_membrane;
    interactive_prompt updated_membrane
  | "3" ->
    let cohere_membrane = apply_cohere_rule membrane in
    print_endline "Updated membrane after applying Cohere rule:";
    print_membrane_state cohere_membrane;
    interactive_prompt cohere_membrane
  | "4" ->
